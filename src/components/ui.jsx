...
export function Button({ variant = "primary", className = "", ...rest }) {
  ...
  return (
    <motion.button
      ...
      className={`inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-2xl px-4 py-2.5 text-sm font-semibold shadow-sm disabled:opacity-50 sm:text-base ${styles[variant]} ${className}`}
      {...rest}
    />
  );
}
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/30 p-0 backdrop-blur-sm sm:items-center sm:p-4"
          onClick={onClose}
        >
          <motion.div
...
export function PageHeader({ title, subtitle, action }) {
  return (
    <motion.div
      ...
      className="mb-6 flex flex-wrap items-center justify-between gap-x-3 gap-y-2"
    >
